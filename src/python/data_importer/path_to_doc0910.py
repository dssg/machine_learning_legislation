...
def path_to_docid0910(earmark_path):
    ...
    if doc_type == 'bill':
        if isinstance(doc_name, tuple):
            doc_ref = doc_name[0]
            div_type = doc_name[1]
            PATH = os.path.join(configuration.get_path_to_bills(), '110/bills/hr/', str(doc_ref))
            all_versions = bill_path.get_all_versions(PATH)
            if 'Div.' in div_type:
                pth_versions = bill_path.get_all_versions(os.path.join(configuration.get_path_to_bills(), '111/bills/hr/hr1105/')
            ...
            if chamber == 'senate':
                pth = os.path.join(configuration.get_path_to_bills(), str(congress), '/bills/s/', str(doc_name))
            else:
                pth = os.path.join(configuration.get_path_to_bills(), str(congress), '/bills/hr/', str(doc_name))
        ...
        if chamber == 'senate':
            pth = os.path.join(configuration.get_path_to_reports(), str(congress), "/senate/", str(doc_name))
        else:
            pth= os.path.join(configuration.get_path_to_reports(), str(congress), "/house/", str(doc_name))
